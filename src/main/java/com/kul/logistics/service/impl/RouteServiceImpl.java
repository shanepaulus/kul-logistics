...
public class RouteServiceImpl implements RouteService {
  ...
  public Route calculateRoute(String origin, String destination) {
    ...
    while (!unvisitedLocations.isEmpty()) {
      ...
      if (currentLocation.getLocationLinkList() != null && !currentLocation.getLocationLinkList()
          .isEmpty()) {
        ...
        for (LocationLink locationLink : currentLocation.getLocationLinkList()) {
          ...
          if (shortestDistanceMap.get(locationLink.getAdjacentLocation())
              > locationLink.getDistance()) {
    ...
    Double totalDistance = getTotalDistance(previousLocationMap, shortestDistanceMap, origin,
        destination);
  ...
  private Double getTotalDistance(Map<String, String> previousLocationMap,
      Map<String, Double> shortestDistanceMap, String origin, String destination) {
    if (!shortestDistanceMap.containsKey(destination)) {
      throw new IllegalStateException(
          "The origin / destination keys was not present in the shortest distance map!");
    } else {
  ...
  private String getShortestDistancePath(Map<String, String> shortestDistanceMap, String origin,
      String destination) {
  ...
  private void preLoadPreviousLocationMap(Map<String, String> previousLocationMap,
      Set<String> locationNameSet) {
    locationNameSet
        .forEach(l -> previousLocationMap.put(l, null));
  }

  private void preLoadShortestDistanceMap(Map<String, Double> shortestDistanceMap,
      Set<String> locationNameSet) {
    locationNameSet
